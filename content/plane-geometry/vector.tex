...
\begin{definition}
  方向相同或相反的向量称为 \emph{共线向量}，约定零向量与任意向量共线. 如果两个向量的大小相等，方向相同，则称这两个向量相等。大小相同，方向相反的两个向量互为\emph{相反向量}.
\end{definition}
...
对于两个向量，还可以引入夹角概念:
\begin{definition}
  通过平移使得两个向量$\bm{\alpha}$和$\bm{\beta}$起点相同，此时两个向量所在两条射线的夹角便称为这两个向量的 \emph{夹角}，记作$<\bm{\alpha},\bm{\beta}>$，其取值范围是$[0,\dfrac{\pi}{2}]$.
\end{definition}
