...
根据前面的向量分解定理，在平面上任意取定两个不共线的向量$\bm{\alpha}$和$\bm{\beta}$，则该平面上的任意向量$\bm{\gamma}$均可以唯一表达成$\bm{\gamma}=x\bm{\alpha}+y\bm{\gamma}$的形式，此时称这两个不共线的向量$\bm{\alpha}$和$\bm{\beta}$为一组\emph{基底向量}，简称\emph{基底}或\emph{基}，并称向量$\bm{\gamma}$在这组基下的\emph{坐标}是$(x,y)$. 同样的概念也可以应用到空间向量身上，此时基底由三个不共面的向量构成，而坐标也就成为三元有序实数组了。
