...
由此引入两个向量的内积概念.
\begin{definition}
  两个向量的 \emph{内积} 是两个向量$\bm{a}$与向量$\bm{b}$的一种二元运算，记为$\bm{a} \cdot \bm{b}$，或者简记为$\bm{a}\bm{b}$，其结果是一个实数，其值规定为：如果$\bm{a}$与$\bm{b}$共线，则其绝对值为两个向量之长度乘积，其符号则由两个向量的方向来决定，方向相同时为正，方向相反时为负，若两者中有零向量，则内积为零；则$\bm{a}$与$\bm{b}$不共线，则两者之内积为其中之一在另一向量上的投影向量与另一向量之内积。
\end{definition}
...
\begin{example}
  对于一个三角形$ABC$和一个定点$O$，由等式$\vv{OP} = \alpha \vv{OA} + \beta \vv{OB} + \gamma \vv{OC}$(或者与其等价的形式$\alpha \vv{PA} + \beta \vv{PB} + \gamma \vv{PC} = \bm{0}$)确定出该三角形所在平面上的一个点，如果三个系数都为非负并且$\alpha+\beta+\gamma = 1$，那么点$P$位于三角形的内部或者三条边上，其中位于内部时三个系数都为正而和为1，位于三条边上时有一个系数为零而另外两个相加为1，当有两个系数为零另外一个为1时正好是顶点之一。
