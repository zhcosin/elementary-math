...
\begin{example}[三角形中的托勒密不等式]
  ...
  \end{equation}
  而如果点$D$是在$BC$边的延长线上，例如是在射线$BC$的延长线上，就成立着:
  \begin{equation}
    \label{eq:ptolemy-inequality-for-triangle-extend}
    AD > -\frac{BC}{AB} \cdot AB + \frac{AC}{AB} \cdot AC
  \end{equation}
  点$D$在射线$CB$延长线上时也有类似结论。

  延长线的那个不等式难以记忆，借助向量就容易记忆了，$AB$与$AC$的系数跟点$D$分$\vv{BC}$的比$\lambda$有关，因为$\vv{BD}=\lambda\vv{DC}$，所以那两个系数其实就是$1/(1+\lambda)$和$\lambda/(1+\lambda)$。

  第一个不等式与四边形中的托勒密不等式极为相似，实际上它就是四边形退化为三角形的情形，这个例子就是说托勒密不等式在这种情况下仍然成立，现在就来证明这个不等式。

  只要证明第一个不等式就行了，当点$D$在射线$BC$的延长线上时，点$C$就在线段$BD$上，这时对三角形$ABD$和$BD$边上的点$C$应用第一个不等式就可以得出第二个不等式。
  ...
  第二个证明就是利用斯特瓦尔特定理，在定理中的等式两边同时除以$BC$，改为如下含比例的等式:
  \begin{eqnarray*}
