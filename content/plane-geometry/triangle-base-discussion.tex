...
\end{figure}
 ...
 值得一提的是，点$E$和$F$分线段$AB$的比为$\lambda$，则点$A$和$B$分线段$EF$的比也相同，这个比值是$\frac{|1-t|}{1+t}$，证明是很容易的，只要把线段$AE$、$EB$、$BF$、$AF$的长度者用$AB$的长度来表示就能得到这一点，这个结论是有用的，比如说任给一个圆和一个不等于1的正实数，我们可以反过来找出它是关于哪两个点的阿波罗尼斯圆，当然这两个点不是唯一的，在每一条直径上都能找出两对。
\end{example}
