...
\begin{figure}[htbp]
  \centering
\includegraphics{content/analytic-geometry/pic/oval-chord-located-in-oval.pdf}
...
\begin{figure}[htbp]
  \centering
\includegraphics{content/analytic-geometry/pic/oval-ray-from-center-split.pdf}
\caption{}
\label{fig:oval-ray-from-center-split}
\end{figure}

\begin{proof}[证明]
  设点$M$是线段$OP$上的任一点，根据我们在例\ref{example:ptolemy-inequality-for-triangle}中所得到的不等式，记$\vv{OM}=\lambda\vv{MP}(\lambda>0)$，有
  \begin{eqnarray*}
    MF_1 < \frac{1}{1+\lambda} OF_1 + \frac{\lambda}{1+\lambda} PF_1 \\
    MF_2 < \frac{1}{1+\lambda} OF_2 + \frac{\lambda}{1+\lambda} PF_2 
  \end{eqnarray*}
  两式相加得
  \begin{eqnarray*}
    MF_1+MF_2 & < & \frac{1}{1+\lambda} (OF_1 + OF_2) + \frac{\lambda}{1+\lambda}  (PF_1 + PF_2) \\
              & = &  \frac{1}{1+\lambda} F_1F_2 + \frac{\lambda}{1+\lambda}  (PF_1 + PF_2) \\
              & < & \frac{1}{1+\lambda} (PF_1+PF_2) + \frac{\lambda}{1+\lambda}  (PF_1 + PF_2) \\
    & = & PF_1 + PF_2
  \end{eqnarray*}
  同样，如果记点$N$是线段$OP$延长线上位于$P$一侧的任一点，把$P$看成$\vv{ON}$的分点，可以得出
  \begin{equation*}
    NF_1 + NF_2 > PF_1 + PF_2
  \end{equation*}
  所以最终
