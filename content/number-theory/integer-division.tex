...
\begin{proof}[证明]
  若$p \mid a$则结论成立，而$p \nmid a$时，由\autoref{theorem:prime-with-any-integer-relatio}知$(p,a)=1$，此时由\autoref{inference:a-prime-b-and-a-mid-bc-so-a-mid-c}知必有$p \mid b$，得证。
\end{proof}
...
\begin{theorem}
  ...
  若$a=p_1p_2 \cdots p_n$，$b=q_1q_2 \cdots q_m$，且诸$p_i(i=1,2,\ldots,n)$及诸$q_j(j=1,2,\ldots,n)$均为素数，并且这两组素数中没有相同的，即不存在$i,j$，使得$p_i=q_j$，那么$(a,b)=1$.
\end{theorem}

\begin{proof}[证明]
 证明这个定理的关键就是\autoref{inference:a-prime-b-and-a-prime-c-so-a-prime-bc}，因为由它，由$(p_i,q_j)=1$，便知$(p_i, b)=1(i=1,2,\ldots,n)$，进一步便有$(a,b)=1$.
\end{proof}
